import 'dart:async';
import 'dart:ffi';
import 'package:news_watch/domain/usecase/health_usecase.dart';
import 'package:rxdart/rxdart.dart';
import 'package:news_watch/domain/model/model.dart';
import 'package:news_watch/presentation/base/base_viewModel.dart';
import 'package:news_watch/presentation/common/state_renderer/state_renderer.dart';
import 'package:news_watch/presentation/common/state_renderer/state_renderer_impl.dart';

class HealthViewModel extends BaseViewModel
    with HealthViewModelInputs, HealthViewModelOutputs {
  StreamController healthStreamController =
      BehaviorSubject<List<ArticleData>>();

  HealthUseCase healthUseCase;
  HealthViewModel(this.healthUseCase);

  @override
  void dispose() {
    healthStreamController.close();
  }

  @override
  void start() {
    _getBusinessDetails();
  }

  _getBusinessDetails() async {
    inputState
        .add(LoadingState(stateRendererType: StateRendererType.LOADING_STATE));
    (await healthUseCase.execute(Void)).fold((failure) {
      inputState.add(ErrorState(
          stateRendererType: StateRendererType.FULLSCREEN_ERROR_STATE,
          message: failure.message));
    }, (details) {
      inputState.add(ContentState());
      inputHealthDetails.add(details.article);
    });
  }

  @override
  // TODO: implement inputPopularDetails
  Sink get inputHealthDetails => healthStreamController.sink;

  @override
  // TODO: implement outputPopularDetails
  Stream<List<ArticleData>> get outputHealthDetails =>
      healthStreamController.stream.map((event) => event);

  @override
  void refresh() async {
    inputState
        .add(LoadingState(stateRendererType: StateRendererType.LOADING_STATE));
    (await healthUseCase.executeIn(Void)).fold((failure) {
      inputState.add(ContentState());
    }, (details) {
      inputState.add(ContentState());
      inputHealthDetails.add(details.article);
    });
  }
}

abstract class HealthViewModelInputs {
  void refresh();
  Sink get inputHealthDetails;
}

abstract class HealthViewModelOutputs {
  Stream<List<ArticleData>> get outputHealthDetails;
}
