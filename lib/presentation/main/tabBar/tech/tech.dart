...
import 'package:news_watch/presentation/main/tabBar/tech/tech_viewModel.dart';
...
class _TechTabState extends State<TechTab> {
  late TechViewModel _viewModel;
  NetworkInfo _networkInfo = instance<NetworkInfo>();

  @override
  void initState()  {
    initTechModule();
    _viewModel = instance<TechViewModel>();
   _call();
    super.initState();
  }
  ...
  _gridData() {
    return StreamBuilder<List<ArticleData>>(
        stream: _viewModel.outputTechDetails,
        builder: (context, snapshot) {
  ...
  Widget _scienceList(List<ArticleData>? data) {
    String screenTitle = AppStrings.tech;
