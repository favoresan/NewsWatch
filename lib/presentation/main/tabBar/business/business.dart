...
import 'package:news_watch/presentation/main/tabBar/business/business_viewModel.dart';
import '../../../../app/di.dart';
...
class _BusinessTabState extends State<BusinessTab> {
  late BusinessViewModel _viewModel;
  NetworkInfo _networkInfo = instance<NetworkInfo>();

  @override
  void initState() {
    initBusinessModule();
    _viewModel = instance<BusinessViewModel>();
    _call();
    super.initState();
  }
  ...
  _gridData() {
    return StreamBuilder<List<ArticleData>>(
        stream: _viewModel.outputBusinessDetails,
        builder: (context, snapshot) {
  ...
  Widget _scienceList(List<ArticleData>? data) {
    String screenTitle = AppStrings.business;
