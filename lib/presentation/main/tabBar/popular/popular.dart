...
class _PopularTabState extends ConsumerState<PopularTab> {
  ...
  void initState() {
    initPopularModule();
    _viewModel = instance<PopularViewModel>();
    _call();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
        ...
        child: Center(child: _popularDetailsView()));
  }

  Widget _popularDetailsView() {
  ...
  _gridData() {
    return StreamBuilder<List<ArticleData>>(
        stream: _viewModel.outputPopularDetails,
        builder: (context, snapshot) {
          return _popularList(snapshot.data);
        });
  }

  Widget _popularList(List<ArticleData>? data) {
    String screenTitle = AppStrings.popular;
