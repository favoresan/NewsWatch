...
import 'package:news_watch/presentation/main/tabBar/entertainment/entertainment_viewModel.dart';
...
class EntertainmentTab extends StatefulWidget {
  const EntertainmentTab({Key? key}) : super(key: key);
...
class _EntertainmentTabState extends State<EntertainmentTab> {
  late EntertainmentViewModel _viewModel;
  NetworkInfo _networkInfo = instance<NetworkInfo>();

  @override
  void initState() {
    initEntertainmentModule();
    _viewModel = instance<EntertainmentViewModel>();
    _call();
    super.initState();
  }
  ...
  _gridData() {
    return StreamBuilder<List<ArticleData>>(
        stream: _viewModel.outputEntertainmentDetails,
        builder: (context, snapshot) {
  ...
  Widget _scienceList(List<ArticleData>? data) {
    String screenTitle = AppStrings.entertainment;
    if (data != null) {
