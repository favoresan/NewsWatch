...
import 'package:news_watch/presentation/main/tabBar/science/science_viewModel.dart';
...
class _ScienceTabState extends State<ScienceTab> {
  late ScienceViewModel _viewModel;
  NetworkInfo _networkInfo = instance<NetworkInfo>();

  @override
  void initState()  {
    initScienceModule();
    _viewModel = instance<ScienceViewModel>();
  _call();
    super.initState();
  }
  ...
  _gridData() {
    return StreamBuilder<List<ArticleData>>(
        stream: _viewModel.outputScienceDetails,
        builder: (context, snapshot) {
  ...
  Widget _scienceList(List<ArticleData>? data) {
    String screenTitle = AppStrings.science;
